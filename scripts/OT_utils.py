...
class OT_measure:
    def __init__(self, with_geomloss, blur):
        ...
        if with_geomloss == 1:
            self.loss_geom = SamplesLoss(
                loss="sinkhorn", p=2, blur=blur, backend="online"
            )
    ...
    def loss(self, anchor_stats, out_stats):
        normalized_std = anchor_stats.std(dim=1)[:, None, :].repeat(
            1, anchor_stats.shape[1], 1
        )
        normalized_mean = anchor_stats.mean(dim=1)[:, None, :].repeat(
            1, anchor_stats.shape[1], 1
        )
        out_stats = (out_stats - normalized_mean) / normalized_std
        anchor_stats = (anchor_stats - normalized_mean) / normalized_std
        loss_geom_list = self.loss_geom(
            anchor_stats.contiguous(), out_stats.contiguous()
        )
