...
def cal_stats_l96(anchor_t, out_t, args=None, index=0):
    ...
    anchor_stats = torch.stack([advection_stats, grad_t, var], dim=-1).reshape(
        b_tz, -1, 3
    )
    out_stats = torch.stack([advection_stats_out, grad_t_out, var_out], dim=-1).reshape(
        b_tz, -1, 3
    )
    return anchor_stats, out_stats


def cal_stats_l1_score(
    anchor_t,
    out_t,
    anchor_param=[1],
    img_name="",
    folder_path="dist_plots",
    bins_test=30,
    apply_gaussian=True,
    calculate_metric=0,
    args=None,
    ...
    def calculate_l1_score(hist_data_anchor, hist_data_predict):
        hist_data_anchor_normalized = (
            hist_data_anchor[0].reshape(-1) / hist_data_anchor[0].reshape(-1).sum()
        )
        hist_data_predic_normalized = (
            hist_data_predict[0].reshape(-1) / hist_data_anchor[0].reshape(-1).sum()
        )

        chi_score = abs(
            hist_data_anchor_normalized - hist_data_predic_normalized
        ).sum()  # + \
        return chi_score
    ...
    var_k_1, var_k_2, var_k_p_1 = (
        torch.from_numpy(var_k_1),
        torch.from_numpy(var_k_2),
        torch.from_numpy(var_k_p_1),
    )
    advection_stats = var_k_1 * (var_k_2 - var_k_p_1)
    ...
    var_k_1_out, var_k_2_out, var_k_p_1_out = (
        torch.from_numpy(var_k_1_out),
        torch.from_numpy(var_k_2_out),
        torch.from_numpy(var_k_p_1_out),
    )
    advection_stats_out = var_k_1_out * (var_k_2_out - var_k_p_1_out)
    ...
    anchor_stats = (
        torch.stack(
            [
                advection_stats.reshape(-1),
                grad_t.reshape(-1),
                torch.from_numpy(var.reshape(-1)),
            ]
        )
        .permute(1, 0)
        .numpy()
    ...
    out_stats = (
        torch.stack(
            [
                advection_stats_out.reshape(-1),
                grad_t_out.reshape(-1),
                torch.from_numpy(var_out.reshape(-1)),
            ]
        )
        .permute(1, 0)
        .numpy()
