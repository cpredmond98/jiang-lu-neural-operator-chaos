...
class CL:
    def __init__(self, args, train_loader_metric, train_sampler, val_loader_metric):
        ...
        if args.distributed:
            ...
            metric_net = torch.nn.parallel.DistributedDataParallel(
                metric_net,
                device_ids=[args.gpu],
        ...
        self.optimizer_metric = torch.optim.AdamW(
            metric_net.parameters(), lr=args.lr_metric, weight_decay=1e-5
        )
        ...
        self.ep_ratio = int(
            args.training_size / (args.batch_size_metricL * args.world_size)
        )
    ...
    def train_metric_net(self, train_vis_pth="", saved_pth_contra=""):
        ...
        for ep in tqdm(np.arange(metric_epochs_train)):
            for param, x, y in tqdm(self.train_loader_metric):
                if ep_real % 1 == 0:
                    ...
                    T_metricL_traj_alone = adjust_tau_metricL(
                        self.T_metricL_traj_alone,
                        ep_real,
                        self.metric_epochs,
                        epochs=100,
                        max_tau_metricL=self.max_tau_metricL,
                    )
                cat_embed = self.metric_net.forward(torch.cat([x, y]).to(self.gpu))
                anchor_embed, pos_anchor_embed = (
                    cat_embed[: self.batch_size_metricL, :],
                    cat_embed[self.batch_size_metricL :, :],
                )
                if self.bank_size > 0:
                    traj_embed_queue = (
                        self.metric_net.module.traj_queue_embed.detach().clone()
                    )
                    pos_anchor_embed_ = torch.cat(
                        [pos_anchor_embed, traj_embed_queue.T], dim=0
                    )
                else:
                ...
                if ep_real % 50 == 0 and ep_real > 0:
                    ...
                    topk_acc = (
                        (
                            (
                                topk_embed
                                == torch.arange(topk_embed.shape[0]).to(self.gpu)[
                                    :, None
                                ]
                            ).sum(dim=-1)
                            > 0
                    ...
                    val_acc = cal_topk_eval(
                        self.val_loader_metric,
                        100,
                        self.metric_net,
                        self.args,
                        pth=train_vis_pth,
        ...
        if self.gpu == 0:
            torch.save(
                {
                    "epoch": ep_real,
                    "state_dict": self.metric_net.module.state_dict(),
                    "optimizer_state_dict": self.optimizer_metric.state_dict(),
                },
                f"{saved_pth_contra}/{int(ep_real):03d}.pth",
            )
    ...
    def calculate_CL_loss(self, y, out):
        ...
        for ey, eo in zip(embed_y[0:], embed_out[0:]):
            embed_dist -= (
                (F.normalize(ey, dim=1) * F.normalize(eo, dim=1)).sum(dim=1).mean()
            )
        ...
        for iter in range(1):
            ...
            topk_acc = (
                (
                    (
                        topk_embed
                        == torch.arange(topk_embed.shape[0]).to(args.gpu)[:, None]
                    ).sum(dim=-1)
                    > 0
                )
                .float()
            ...
            topk_acc_5 = (
                (
                    (
                        topk_embed_5
                        == torch.arange(topk_embed_5.shape[0]).to(args.gpu)[:, None]
                    ).sum(dim=-1)
                    > 0
                )
                .float()
