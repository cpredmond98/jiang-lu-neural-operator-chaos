...
def main_worker(gpu, ngpus_per_node, args):
    ...
    prefix_for_CL, contra_models_path, operators_path, output_path = create_folder_path(
        args
    )
    ...
    if args.metric_epochs > 0:
        ...
        if args.train_metric:
            ...
            metric_net = CL.load_metric_net(
                f"{saved_pth_contra}/{args.metric_epochs:03d}.pth", args.gpu
            )
        if args.is_master:
            ...
            val_acc = cal_topk_eval(
                val_loader_metric, val_btz, CL.metric_net, args, pth=output_path
            )
    ...
    optimizer = torch.optim.AdamW(
        operator.parameters(), lr=args.learning_rate, weight_decay=1e-4
    )
    train_epoch_ = 0
    if args.distributed:
    ...
    if args.train_operator:
        ...
        for ep in tqdm(range(epochs)):
            ...
            for param, y in train_loader_operator:
                operator.train()
                l2, loss_OT, loss_CL = (
                    torch.tensor([0]).to(args.gpu).float(),
                    torch.tensor([0]).to(args.gpu).float(),
                    torch.tensor([0]).to(args.gpu).float(),
                )
                lr_ = adjust_learning_rate_cos(
                    args.learning_rate, optimizer, ep, epochs, args
                )
                ...
                y_predict = long_length_predict_with_yinit(
                    operator, y, param, args.x_len, args.len_to_operator
                )
                ...
                if args.with_geomloss > 0 and ep >= 20:
                    if args.l96:
                        anchor_stats, out_stats = cal_stats_l96(
                            y.squeeze(),
                            y_predict.squeeze(),
                            args=args,
                            index=args.with_geomloss_kd,
                        )
                        if args.with_geomloss_kd != 0:
                            anchor_stats, out_stats = (
                                anchor_stats[
                                    :, :, np.array([args.with_geomloss_kd - 1])
                                ],
                                out_stats[:, :, np.array([args.with_geomloss_kd - 1])],
                            )
                ...
                optimizer.zero_grad()
                loss = (
                    l2 + args.lambda_contra * loss_CL + args.lambda_geomloss * loss_OT
                )
                loss.backward()
                optimizer.step()
                ep_loss.append(
                ...
                loss_list.append(
                    [
                        np.array(ep_loss).mean(axis=0)[0],
                        np.array(ep_loss).mean(axis=0)[1],
            ...
            if ep % 50 == 0 and ep > 0:
                visualiztion(
                    ...
                    operator,
                    args,
                    img_pth=f"{output_path}/training_vis",
                    ep=ep,
                )
        ...
        if ep == epochs - 1:
            if args.is_master:
                save_operator(
                    operator,
                    optimizer,
                    saved_pth=f"{operators_path}/{args.prefix}/{ep:03d}",
                    ep=ep,
                )
    ...
    operator = load_operator(
        operator, saved_pth=f"{operators_path}/{args.prefix}/{ep:03d}"
    )
    visualiztion(
        train_dataset_operator,
        operator,
