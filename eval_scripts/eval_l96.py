...
def eval_l96(
    operator, args, noisy_scale=0, x_len=100, calculate_l2=True, output_path=""
):
    ...
    if noisy_scale == 0:
        ...
        TestingData_Initial = TestingData(
            crop_T, noisy_scale=args.noisy_scale, convert_to_pil=False
        )
        TestingData_eval = TestingData(
            crop_T, noisy_scale=noisy_scale, convert_to_pil=False
        )
    else:
        eval_size = 100
        TestingData_Initial = TrainingData(
            crop_T,
    ...
    del all_data_eval
    params_initial, data_initial = (
        params_initial[:eval_size],
        data_initial[:eval_size].squeeze(),
    )
    ...
    for x_0_start in tqdm(x_0_start_list):
        ...
        with torch.no_grad():
            ...
            for idata in range(eval_size):
                ...
                bins_ = 0
                slow_stats = cal_stats_l1_score(
                    x.cpu().data.numpy(), out.cpu().data.numpy()
                )
        ...
        spectrum_stats = [
            np.quantile(np.array(dist_spectrum), q=0.25),
            np.array([50]).reshape(-1, 1),
            np.quantile(np.array(dist_spectrum), q=0.5),
            np.quantile(np.array(dist_spectrum), q=0.75),
        ]
        ...
        torch.save(
            {"out": out_list, "x": x_list, "slow_stats_array": slow_stats_array},
            save_path,
        )
        l1_score_3d = [
            slow_stats_array.mean(axis=0),
            np.quantile(slow_stats_array, q=0.25),
            "50 percentile",
            np.quantile(slow_stats_array, q=0.5),
            np.quantile(slow_stats_array, q=0.75),
        ]
        del out_list, x_list
    ...
    with torch.no_grad():
        ...
        for b_ix in range(int(eval_size / eval_bsize)):
            ...
            params_eval_btz = params_eval[
                eval_bsize * b_ix : eval_bsize * (b_ix + 1), 0
            ].reshape(eval_bsize, -1)
            data_eval_btz = data_eval[eval_bsize * b_ix : eval_bsize * (b_ix + 1)]
            x_0 = data_initial_btz[:, :-1].reshape(
                eval_bsize * (data_initial_btz.shape[1] - 1), -1
            )
            params = params_eval_btz.repeat(1, data_initial_btz.shape[1] - 1).reshape(
                eval_bsize * (data_initial_btz.shape[1] - 1), -1
            )
            x_1_true = (
                data_eval_btz[:, 1:]
                .reshape(eval_bsize * (data_initial_btz.shape[1] - 1), -1)
                .to(args.gpu)
            )
    ...
    l2_list_stats = [
        "rMSE",
        np.quantile(np.array(l2_lp_list), 0.25),
        "50 percentile",
        np.quantile(np.array(l2_lp_list), 0.5),
        np.quantile(np.array(l2_lp_list), 0.75),
    ]
    ...
    if noisy_scale == 0:
        with open(f"{output_path}/Results_test_on_clean_data.txt", "w") as f:
            f.writelines(
                f"noise {noisy_scale} with eval length {x_len} training length {args.x_len} \n"
            )
            f.writelines(f"mse_{mse_step} \n ")
            f.writelines(
                f"l1_3d_score: {l1_score_3d} \n spectrum distance:{spectrum_stats} \n \n"
            )
    else:
