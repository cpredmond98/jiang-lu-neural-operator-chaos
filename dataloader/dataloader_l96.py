...
def filter_invalid_index(data_list, params_array, invalid_idx):
    ...
    all_idx = np.array(
        [int(data_path.split("/")[-1].split(".")[0]) for data_path in data_list]
    )
...
def random_cond_contra_cropping(traj, crop_T, no_align=True):
    ...
    if no_align:
        ...
        Tsidx_2 = torch.tensor(
            [out_index[torch.randint(out_index.shape[0], size=(1, 1))[0]]]
        )
...
class TrainingData(Dataset):
    def __init__(
        ...
        if self.convert_to_pil:
            ...
            self.data_list.sort()
            self.params_array_list = torch.load(
                "{}/training_params.pth".format(self.data_path)
            )
        else:
            self.data_list = glob.glob(
                "{}/00*_noise_{:.2f}.tiff".format(data_path, self.noisy_scale)
            )
            print("use {} data".format(noisy_scale), self.data_list[0])
            self.data_list.sort()
            self.params_array_list = torch.load(
                "{}/training_params.pth".format(data_path)
            )
            self.nan_list = torch.load("{}/nan_list.pth".format(data_path))
            self.data_list, self.params_array_list = filter_invalid_index(
                self.data_list, self.params_array_list, self.nan_list
            )
            self.data_list = self.data_list[:train_size]
            self.params_array_list = self.params_array_list[:train_size]
            print(
                "after filter nan data, we have {} size data".format(
                    len(self.data_list)
                )
            )
        ...
        if self.infinite_data_loader:
            self.infinite_data_idx = torch.cat(
                [
                    torch.randperm(len(self.data_list))[:total_batch_size]
                    for _ in range(self.total_epoch)
                ]
            )
            self.infinite_data_idx = self.infinite_data_idx.cuda(
    ...
    def __getitem__(self, idx):
        ...
        if self.convert_to_pil:
            ...
            anchor_param, anchor_t = (
                data_1d_recon[:num_of_param],
                data_1d_recon[num_of_param:].reshape(T, dim),
            )
            if self.train_operator:
                if self.crop_T < self.T:
                    crop = random_cropping(
                        torch.from_numpy(anchor_t[None, :, :]), self.crop_T
                    )
                else:
                    crop = torch.from_numpy(anchor_t)
                return anchor_param, crop
            else:
                crop_1, crop_2 = random_cond_contra_cropping(
                    torch.from_numpy(anchor_t[None, :, :]), self.crop_T, True
                )
...
class TestingData(Dataset):
    def __init__(self, crop_T, noisy_scale=0, convert_to_pil=False, train_size=1000):
        ...
        if self.convert_to_pil:
            ...
            self.data_list = glob.glob(
                "{}/00*_noise_{:.2f}.tiff".format(self.data_path, self.noisy_scale)
            )
        self.data_list.sort()
...
if __name__ == "__main__":
    ...
    def save_data_with_pil(dataset):
        ...
        for i in range(len(dataset)):
            ...
            noise_traj_x = (
                noisy_scale * std_x * (torch.randn(traj_x.shape, device=traj_x.device))
            ).squeeze()
            ...
            if save_pil:
                ...
                data_1d = np.concatenate(
                    [anchor_param.reshape(-1), anchor_t.reshape(-1).astype(np.float32)]
                )
                img_data = Image.fromarray(data_1d)
                img_data.save(
                    "{}/{:06d}_noise_{:.2f}.tiff".format(
                        dataset.data_path, idx, noisy_scale
                    )
                )
        if save_pil:
    ...
    noisy_scale = 0.3
    training_data_noisy = TrainingData(
        crop_T=crop_T,
        convert_to_pil=True,
        noisy_scale=noisy_scale,
        train_size=train_size,
    )
    validation_data_noisy = TrainingData(
        crop_T=crop_T,
        convert_to_pil=True,
        noisy_scale=noisy_scale,
        train_size=train_size,
    ...
    testing_data_noisy = TestingData(
        crop_T=crop_T, convert_to_pil=True, noisy_scale=noisy_scale
    )
